...
| `Steam Wisp A`, `Steam Wisp B`, `Steam Wisp C` | Animated steam above the kettle. |
| `Queue Customer A`, `Queue Customer B`, `Queue Customer C` body/head/hair objects | Small customer silhouettes beside the counter. |
| `Customer Queue Shadow` | Grounding shadow for the queue cluster. |
| `UPI QR Prop Backplate`, `UPI QR Prop Header`, `UPI QR Prop Code Field` | Counter payment sign. |
| `UPI QR Finder A/B/C` and `UPI QR Dot 01-06` | QR-like payment pattern. |

The current art is procedural Unity UI geometry generated by `ChaiGamePresenter`, with no imported texture files.

Steam behavior:

- Wisps rise from the kettle lid.
- Wisps subtly sway side to side.
- Wisps scale up and fade out over a repeating lightweight cycle.

Customer queue behavior:

- Three static silhouettes sit near the right side of the counter.
- Each customer is made from procedural UI shapes so there are no imported character textures.

UPI QR prop behavior:
